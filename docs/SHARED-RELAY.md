...
- The guild administrator can set the minimum rarity, always-track IDs, and blocked IDs. Save applies to all relays immediately; no restart is needed. Blocked IDs override the watch list.
...
The addon captures all own loot so shared filters can change freely. In 0.3, `/lootbot quality` and `/lootbot watch` point to the app's shared controls. This guild initially remains in all-loot testing mode; switch minimum rarity to Epic for normal use.
